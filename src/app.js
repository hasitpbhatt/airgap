...
function init() {
  ...
  if (state.getState().settings.currentPersona === 'custom') {
    ...
    ui.elements.systemPromptTextarea.value = PERSONAS[state.getState().settings.currentPersona].system;
...
window.selectPersonaForNewChat = (personaName) => {
  ui.selectPersonaForNewChat(personaName);
};

window.retryMessage = (errorIndex) => {
  ui.retryMessage(errorIndex);
};

window.copyMessageText = (button, index) => {
  ui.copyMessageText(button, index);
};

window.editUserMessage = (index) => {
